#!/bin/bash
# auth：lizhiquan (Final Fixed Version)
# email：zqli6@qq.com
# 功能：多主机免密认证（支持自动补全IP、双密码尝试、自定义用户、权限修复、输入校验）
        ...
        case "$CH" in
            ...
            [yY]*)
                ...
                if command -v apt > /dev/null 2>&1; then
                    ...
                    yum install -y ping expect &> /dev/null
                elif command -v dnf > /dev/null 2>&1; then
                    dnf install -y iputils expect &> /dev/null
                else
...
# 5. 密钥分发函数 (修复语法错误和逻辑)
ssh_copy_id() {
    # 直接使用全局变量 PUB_KEY_PATH，不再重复 ls 获取，避免语法错误和冗余
...
# 如果 collect_ips 中已经做了空数组检查，这里再次确认
if [ ${#HOSTS[@]} -eq 0 ]; then
    echo -e "\e[31m无有效 IP 地址，退出\e[0m"
    exit 1
fi
